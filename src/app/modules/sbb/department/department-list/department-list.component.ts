...
import { DepartmentDataSource } from './department-data-source';
...
export class DepartmentListComponent implements OnInit {
    ...
    constructor(
        ...
        this.dataSource = new DepartmentDataSource(this.departmentService, this.mappingModels);
    }
    ...
    addDepartment() {
        this.router.navigateByUrl('/sbb/create-department');
    }

    openEdit(element: any) {
        this.router.navigateByUrl(`/sbb/departments/${element.id}/update`);
    }

    setActive(element: any) {
        this.departmentService.setActive(element.id).subscribe(() => {
            this.alertService.success(`Bộ phận ${element.name} ở trạng thái hoạt động`);
            this.loadPage();
        }, (err) => {
    ...
    setDisable(element: any) {
        this.departmentService.setInactive(element.id).subscribe(() => {
            this.alertService.success(`Bộ phận ${element.name} ở trạng thái không hoạt động`);
            this.loadPage();
        }, (err) => {
            this.alertService.showToastError();
            console.log(err);
    ...
    openDetail(element: any){
        this.router.navigateByUrl(`/sbb/departments/${element.id}`);
    }
